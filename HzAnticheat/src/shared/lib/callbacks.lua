...
function Callbacks.initialize(isServer)
    if isServer then
        RegisterNetEvent("HzAnticheat:callback:serverRequest", function(eventName, callbackId, ...)
            local src = source
            
            if Callbacks.callbackRegistry[eventName] then
                local result = Callbacks.callbackRegistry[eventName](src, ...)
                TriggerClientEvent("HzAnticheat:callback:clientResponse", src, callbackId, result)
            end
        end)
    else
        RegisterNetEvent("HzAnticheat:callback:clientResponse", function(callbackId, ...)
...
function Callbacks.triggerServerCallback(name, cb, ...)
    ...
    TriggerServerEvent("HzAnticheat:callback:serverRequest", name, Callbacks.currentCallbackId, ...)
end
