...
function DiscordLogger.initialize(config)
    ...
    if DiscordLogger.enabled then
        DiscordLogger.log_system(
            "System Started", 
            "The HzAnticheat Anti-Cheat system has been initialized.",
            {
                {name = "Status", value = "✅ Online", inline = true},
                {name = "Version", value = "HzAnticheat v1.0", inline = true}
            }
        )
    end
...
function DiscordLogger.send(webhook_type, title, description, fields, image_url, footer_text, thumbnail_url)
    ...
    local embed = {
        ...
        fields = fields or {},
        footer = {
            text = footer_text or "HzAnticheat Anti-Cheat • " .. formattedDateTime
        }
    }
    ...
    local payload = {
        username = "HzAnticheat Anti-Cheat",
        embeds = { embed }
    }
