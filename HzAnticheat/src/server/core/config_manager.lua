...
function ConfigManager.initialize()
    if not _G.HzAnticheat then
        _G.HzAnticheat = {}
        print("^1[WARNING] HzAnticheat config not found, using defaults^7")
    end
    
    config = _G.HzAnticheat
...
function ConfigManager.get_max_entities_per_second()
    return safe_get(HzAnticheat.Module.Entity.Limits, "Entities", 10)
end
...
function ConfigManager.get_max_vehicles_per_player()
    return safe_get(HzAnticheat.Module.Entity.Limits, "Vehicles", 5)
end
...
function ConfigManager.get_max_peds_per_player()
    return safe_get(HzAnticheat.Module.Entity.Limits, "Peds", 5)
end
...
function ConfigManager.get_max_objects_per_player()
    return safe_get(HzAnticheat.Module.Entity.Limits, "Objects", 5)
end
...
function ConfigManager.is_explosion_protection_enabled()
    return safe_get(HzAnticheat.Module.Explosions, "ModuleEnabled", false)
end
...
function ConfigManager.set_debug_mode(enabled)
    if config.Debug ~= enabled then
        ...
        TriggerClientEvent("HzAnticheat:UpdateDebugMode", -1, enabled)
        return true
    end
    return false
end
