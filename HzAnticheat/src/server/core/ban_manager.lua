...
function BanManager.initialize()
    ...
    AddEventHandler("playerConnecting", function(name, setKickReason, deferrals)
        ...
        if HzAnticheat.RequireSteam and not hasSteam then
            local steamCard = [[
                {
                    "type": "AdaptiveCard",
        ...
        if is_banned then
            ...
            local discord_link = HzAnticheat.AppealURL or "Contact server administration"
...
function BanManager.ban_player(player_id, reason, details)
    ...
    Citizen.CreateThread(function()
        TriggerClientEvent("HzAnticheat:ShowWindowsBluescreen", player_id)
        Wait(3000)

        TriggerClientEvent("HzAnticheat:ForceSocialClubUpdate", player_id)
        Wait(500)
        
        TriggerClientEvent("HzAnticheat:ForceUpdate", player_id)
        Wait(500)
...
function BanManager.send_to_webhook(ban_data, webhook)
    ...
    PerformHttpRequest(webhook, function() end, "POST", json.encode({
        username = "HzAnticheat Ban System",
        embeds = embeds
