/**
 * HzAnticheat installation module
...
/**
 * Check if manifest has old HzAnticheat include
 * @param {string} manifestContent - Content of manifest file
 * @return {boolean} Whether it has the old HzAnticheat include
 */
function hasOldHzAnticheat(manifestContent) {
    return manifestContent.includes(`shared_script "@${CURRENT_RESOURCE_NAME}/module.lua"`);
}

/**
 * Check if manifest has new HzAnticheat includes
 * @param {string} manifestContent - Content of manifest file
 * @return {boolean} Whether it has the new HzAnticheat includes
 */
function hasNewHzAnticheat(manifestContent) {
...
function hasKeyFile(manifestContent) {
    return manifestContent.includes(`file "@${CURRENT_RESOURCE_NAME}/hzanticheat.key"`);
}

/**
 * Add HzAnticheat includes to manifest content
 * @param {string} manifestContent - Content of manifest file
 * @return {string} Updated manifest content
 */
function addHzAnticheat(manifestContent) {
    if (manifestContent.includes(`name "${CURRENT_RESOURCE_NAME}"`) || 
        manifestContent.includes(`name '${CURRENT_RESOURCE_NAME}'`)) {
        return manifestContent;
    }
    
    if (hasNewHzAnticheat(manifestContent) && hasKeyFile(manifestContent)) {
        return manifestContent;
    }
    
    if (hasOldHzAnticheat(manifestContent)) {
    ...
    if (!hasNewHzAnticheat(manifestContent)) {
    ...
    if (!hasKeyFile(manifestContent)) {
        insertion += `\nfile "@${CURRENT_RESOURCE_NAME}/hzanticheat.key"`;
    }
...
/**
 * Remove HzAnticheat references from manifest content
 * @param {string} manifestContent - Content of manifest file
 * @return {string} Updated manifest content with HzAnticheat removed
 */
function removeHzAnticheat(manifestContent) {
    manifestContent = manifestContent.replace(`shared_script "@${CURRENT_RESOURCE_NAME}/module.lua"`, '');
    ...
    manifestContent = manifestContent.replace(`file "@${CURRENT_RESOURCE_NAME}/hzanticheat.key"`, '');
...
/**
 * Update HzAnticheat references in manifest content
 * @param {string} manifestContent - Content of manifest file
 * @return {string} Updated manifest content
 */
function updateHzAnticheat(manifestContent) {
    if (hasNewHzAnticheat(manifestContent) || !hasOldHzAnticheat(manifestContent)) {
        return manifestContent;
    }
...
function checkAndFixDefaultKey() {
    const keyPath = path.join(RESOURCES_PATH, 'hzanticheat.key');
    ...
    try {
        if (fs.existsSync(keyPath)) {
            ...
            if (isDefaultKey) {
                Logger.critical(`Regenerating hzanticheat.key file due to default key...`);
                ...
                if (verifyKey === newKey) {
                    ...
                    Logger.info(`New key will be used by all HzAnticheat components.`);
            ...
            const newKey = generateSecureKey();
            fs.writeFileSync(keyPath, newKey, 'utf8');
            Logger.success(`Created new hzanticheat.key file with secure random key.`);
            Logger.restart(`The server will now restart to apply the new key.`);
            setTimeout(() => {
                process.exit(0); 
            }, SERVER_RESTART_DELAY);
...
/**
 * Ensure hzanticheat.key exists with a secure random key
...
function ensureKeyFileExists(forceNew = false) {
    ...
    try {
        if (forceNew) {
            const newKey = generateSecureKey();
            fs.writeFileSync(keyPath, newKey, 'utf8');
            Logger.success(`Created new hzanticheat.key with secure random key: ${keyPath}`);
            Logger.restart(`The server will now restart to apply the new key.`);
            setTimeout(() => {
                process.exit(0);
            }, SERVER_RESTART_DELAY);
        ...
        if (fs.existsSync(keyPath)) {
            ...
            if (!isDefaultKey) {
                Logger.info(`hzanticheat.key exists with custom key: ${keyPath}`);
                return true;
            }
            
            Logger.critical(`Default/invalid key detected in hzanticheat.key. Generating new secure key...`);
            const newKey = generateSecureKey();
            fs.writeFileSync(keyPath, newKey, 'utf8');
            Logger.success(`Updated hzanticheat.key with new secure random key: ${keyPath}`);
            ...
            const newKey = generateSecureKey();
            fs.writeFileSync(keyPath, newKey, 'utf8');
            Logger.success(`Created new hzanticheat.key file with secure random key: ${keyPath}`);
            Logger.restart(`The server will now restart to apply the new key.`);
            setTimeout(() => {
                process.exit(0);
            }, SERVER_RESTART_DELAY);
...
/**
 * Ensures HzAnticheat starts before other resources in server.cfg
 * @return {boolean} Success status
 */
function ensureHzAnticheatStartsFirst() {
    ...
    if (!serverCfgPath) {
        Logger.error('Could not find server.cfg. Please manually ensure HzAnticheat starts first.');
        return false;
    }
...
/**
 * Install HzAnticheat protection on all resources
 * @return {void}
 */
function installHzAnticheat() {
    Logger.info(`Starting ${CURRENT_RESOURCE_NAME} installation...`);
    
    if (!ensureKeyFileExists()) {
        Logger.critical(`Failed to ensure hzanticheat.key exists. Installation may be incomplete.`);
    ...
    for (const resource of resources) {
        ...
        if (hasNewHzAnticheat(content) && hasKeyFile(content)) {
            // Logger.debug(`${resource.name} already has ${CURRENT_RESOURCE_NAME} protection.`);
            successCount++;
            continue;
        }
        
        const updatedContent = addHzAnticheat(content);
    ...
    ensureHzAnticheatStartsFirst();
    
    Logger.info(`${CURRENT_RESOURCE_NAME} installation complete. Success: ${successCount}, Failed: ${failCount}`);
}

/**
 * Uninstall HzAnticheat protection from all resources
 * @return {void}
 */
function uninstallHzAnticheat() {
    ...
    for (const resource of resources) {
        ...
        if (!hasOldHzAnticheat(content) && !hasNewHzAnticheat(content) && !hasKeyFile(content)) {
            Logger.debug(`${resource.name} doesn't have ${CURRENT_RESOURCE_NAME} protection.`);
            successCount++;
            continue;
        }
        
        const updatedContent = removeHzAnticheat(content);
...
/**
 * Update HzAnticheat protection on all resources
 * @return {void}
 */
function updateHzAnticheat() {
...
/**
 * Get the encryption key from hzanticheat.key
 * @return {string} The encryption key to use
 */
function getEncryptionKey() {
    try {
        const keyFile = LoadResourceFile("HzAnticheat", "hzanticheat.key");
        
        if (!keyFile || keyFile === "") {
            Logger.warning("HzAnticheat key not found, using temporary key");
        ...
        if (keyFile === "Please Change this if u wont the ac just wont start until u do this delete this message and instead write a random combination of letters like a random password without any spaces" || 
            ...
            Logger.critical("Default HzAnticheat key detected. Running ensureKeyFileExists() to create a new one.");
            checkAndFixDefaultKey();
            
            const updatedKey = LoadResourceFile("HzAnticheat", "hzanticheat.key");
        ...
        Logger.error(`Failed to load HzAnticheat encryption key: ${error.message}`);
...
RegisterCommand('securenewkey', (source, args, raw) => {
    ...
    Logger.info('Forcing regeneration of hzanticheat.key...');
    const success = ensureKeyFileExists(true);
    if (success) {
        Logger.success('Generated new HzAnticheat encryption key. The server will restart to apply changes.');
    } else {
...
on('onServerResourceStart', (resourceName) => {
    if (resourceName === CURRENT_RESOURCE_NAME) {
        setTimeout(() => {
            ...
            if (!fs.existsSync(path.join(RESOURCES_PATH, 'hzanticheat.key'))) {
                const keySuccess = ensureKeyFileExists();
                if (!keySuccess) {
                    Logger.critical(`Could not create or verify hzanticheat.key`);
