...
function AutoConfig.process_auto_whitelist(src, reason, webhook, time)
    if not HzAnticheat.AutoConfig then
    ...
    if detectedResource and detectedResource == "HzAnticheat" then
    ...
    if isEvent and detectedResource then
        ...
        local newConfig, success = AutoConfig.append_to_table(configFile, "HzAnticheat.Module.Events.Whitelist", isEvent)
        if success then
    ...
    elseif isUnregisteredEvent then
        ...
        local newConfig, success = AutoConfig.append_to_table(configFile, "HzAnticheat.Module.Events.Whitelist", isUnregisteredEvent)
        if success then
            SaveResourceFile(GetCurrentResourceName(), "config.lua", newConfig, -1)
    ...
    elseif isSuspiciousEntity then
        ...
        local newConfig, success = AutoConfig.add_entity_resource(configFile, "HzAnticheat.EntitySecurity", isSuspiciousEntity)
        if success then
            SaveResourceFile(GetCurrentResourceName(), "config.lua", newConfig, -1)
    ...
    elseif detectedResource then
        ...
        local newConfig, success = AutoConfig.add_entity_resource(configFile, "HzAnticheat.EntitySecurity", detectedResource)
        if success then
            SaveResourceFile(GetCurrentResourceName(), "config.lua", newConfig, -1)
...
function AutoConfig.validate_event(src, event_name, resource_name, webhook)
    ...
    if HzAnticheat.AutoConfig then
