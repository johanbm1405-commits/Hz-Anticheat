...
function AntiExplosions.initialize()
    if not HzAnticheat.Module.Explosions.ModuleEnabled then
        return
    end
    ...
    RegisterNetEvent("HzAnticheat:Explosions:Whitelist", function(data)
    ...
    AddEventHandler('explosionEvent', function(sender, ev)
        ...
        if GetPlayerPing(sender) > 0 and HzAnticheat.ExplosionsModule then
            if whitelist[sender] or HzAnticheat.ExplosionsWhitelist[resourceName] then
        ...
        for k, v in pairs(HzAnticheat.Protection.BlacklistedExplosions) do
            if ev.explosionType == v.id then
                ...
                if HzAnticheat.Protection.CancelOtherExplosions then
            for k, v in pairs(HzAnticheat.Protection.BlacklistedExplosions) do
                        if ev.explosionType ~= v.id then
                            CancelEvent()
                        end
