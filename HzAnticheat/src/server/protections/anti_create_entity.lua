...
function AntiCreateEntity.initialize()
    if HzAnticheat and HzAnticheat.Module and HzAnticheat.Module.Entity and HzAnticheat.Module.Entity.SecurityWhitelist then
        for _, entry in ipairs(HzAnticheat.Module.Entity.SecurityWhitelist) do
            AntiCreateEntity.resourceWhitelist[entry.resource] = entry.whitelist
        end
        logger.info("Loaded " .. #HzAnticheat.Module.Entity.SecurityWhitelist .. " whitelisted resources for entity security")
    end

    RegisterNetEvent("HzAnticheat:Server:Methods:ModulePunish", function(screenshot, reason, webhook, time)
        local src = source
        if not src or src <= 0 then return end
        
        logger.warn(string.format("[HzAnticheat] Entity Security: Player %s (%s) %s", 
        ...
        print("[HzAnticheat] Incase this is a false ban go to config.lua and in it search HzAnticheat.Module incase this is an event ban go to Events and then to whitelist and add the event")
        print("[HzAnticheat] Incase its an entity ban go to Entity and SecurityWhitelist and add the resource name")
        if not screenshot and HzAnticheat.Module.Entity.TakeScreenshot then
    ...
    RegisterNetEvent("HzAnticheat:Server:Methods:Entity:CreateServer", function(entityId, resourceName, modelHash)
        local src = source
        AntiCreateEntity.allowHash(modelHash)
    end)

    RegisterNetEvent("HzAnticheat:Server:Methods:Entity:Create", function(entityId, resourceName, modelHash)
        local src = source
    ...
    AddEventHandler('entityCreated', function(entity)
        ...
        if (population == 7 or population == 0) and owner ~= 0 and owner ~= -1 then
            ...
            elseif owner and modelHash then
                TriggerClientEvent("HzAnticheat:CheckEntityResource", owner, NetworkGetNetworkIdFromEntity(entity), modelHash)
            end
        end
    end)
