...
function ResourceManager.initialize()
    RegisterNetEvent("HzAnticheat:Server_Callbacks:Protections:GetResourceStatus", function()
        local src = source
        ...
        TriggerClientEvent("HzAnticheat:Client_Callbacks:Protections:GetResourceStatus", src, stopped, started, restarted)
    end)
    ...
    AddEventHandler("onResourceStop", function(resource_name)
        if resource_name == GetCurrentResourceName() then
            for _, player in pairs(GetPlayers()) do
                -- DropPlayer(player, "HzAnticheat anticheat was stopped.")
            end
        end
    end)
end
