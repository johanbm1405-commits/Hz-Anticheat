...
function AntiExecution.initialize()
    RegisterNetEvent("HzAnticheat:Server_Callbacks:Detections:RegisterKnownMenus", function(menus)
    ...
    AddEventHandler("playerConnecting", function(_, _, deferrals)
        ...
        if GetPlayerEndpoint(source) == nil then
            deferrals.done("HzAnticheat: Invalid connection detected.")
            CancelEvent()
        else
            deferrals.update("HzAnticheat: Checking connection...")
            deferrals.done()
        end
    end)
end
