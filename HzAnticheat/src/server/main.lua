...
local function setupErrorHandler()
    HzAnticheatErrorHandler = function(err)
    ...
    AddEventHandler('onServerResourceStart', function(resource)
        if resource == GetCurrentResourceName() then
            ...
            error = function(err, level)
                if HzAnticheatErrorHandler then
            HzAnticheatErrorHandler(err)
                end
...
local function registerServerCommands()
    ...
    RegisterCommand("securebanlist", function(source, args, rawCommand)
        ...
        print("===== HzAnticheat Ban List (" .. #bans .. " most recent) =====")
    ...
    RegisterCommand("securehelp", function(source, args, rawCommand)
        ...
        print("===== HzAnticheat Anti-Cheat Commands =====")
    ...
    RegisterCommand("securestats", function(source, args, rawCommand)
        ...
        print("===== HzAnticheat System Statistics =====")
...
local function main()
 ...
 |_| |_/____|    /_/   \_\_| |_|\__|_|\___|_| |_|\___|\__,_|\__|   
  ...
  ^7]])
    ...
    print("^8╔══════════════════════════════════════════════════════════════════════════╗^7")
    print("^8║                  ^2HzAnticheat AntiCheat v1.2.1 Initializing^8               ║^7")
    print("^8╚══════════════════════════════════════════════════════════════════════════╝^7")
    ...
    logger.initialize(HzAnticheat)
    logger.initialize({
        LogLevel = HzAnticheat.LogLevel,
        UseWebhook = HzAnticheat.UseWebhook,
        LogWebhook = HzAnticheat.LogWebhook,
        MaxLogHistory = HzAnticheat.MaxLogHistory,
        Debug = HzAnticheat.Debug
    })
    print("^2│ ^2✓^7 Logger^7 initialized")

    print("^2│ ^5⏳^7 Discord Logger^7")
    discord_logger.initialize(HzAnticheat)
    print("^2│ ^2✓^7 Discord Logger^7 initialized")
    print("^2│ ^5⏳^7 Debug Module^7")
    debug_module.initialize(HzAnticheat)
    ...
    -- Asignar globalmente para que esté disponible en las pruebas
    ...
    AddEventHandler("onResourceStop", function(resource_name)
        if resource_name == GetCurrentResourceName() then
            logger.info("HzAnticheat AntiCheat is stopping...")
            logger.warn("HzAnticheat AntiCheat is stopping...")
        end
    end)
    ...
    AddEventHandler("eventTriggered", function(event_name, source, ...)
        if HzAnticheat.AutoConfig and auto_config and auto_config.process_auto_whitelist then
            auto_config.process_auto_whitelist(source, "Event triggered: " .. event_name, nil, nil)
        end

        if event_name and HzAnticheat.SafeEvents then
            local safe_events = HzAnticheat.SafeEvents
    ...
    AddEventHandler("resourceStarted", function(resource_name)
        ...
        if resource_to_check and HzAnticheat.SafeResources then
            local safe_resources = HzAnticheat.SafeResources
    ...
    RegisterNetEvent("HzAnticheat:ClientLog", function(level, message)
        local source = source
        local player_name = GetPlayerName(source) or "Unknown"
    ...
    initialized = true
    print("\n^8╔══════════════════════════════════════════════════════════════════════════╗^7")
    print("^8║              ^2HzAnticheat AntiCheat v1.2.1 Loaded Successfully^8            ║^7")
    ...
    discord_logger.log_system(
        "AntiCheat Started",
        "HzAnticheat AntiCheat v1.2.1 has been successfully initialized.",
        {
    ...
    logger.info("HzAnticheat AntiCheat v1.2.1 initialized successfully")
end
...
exports("module_punish", function(source, reason, webhook, time)
    ...
    if resource_to_check then
        if resource_to_check == GetCurrentResourceName() then
            logger.debug("Detection from HzAnticheat itself, ignoring")
            return true
        end
...
RegisterNetEvent("HzAnticheat:DisconnectMe", function()
    local source = source
...
RegisterNetEvent("HzAnticheat:Server:Methods:PunishPlayer", function(id, reason, webhook, time)
    local source = source
