...
function getEncryptionKey() {
    try {
        ...
        if (!keyFile || keyFile === "") {
            console.warn("[WARNING] Failed to load HzAnticheat encryption key. Using temporary key.");
            return "temp_key_" + GetCurrentResourceName();
        }
        
        return keyFile.trim();
    } catch (error) {
        console.error("[ERROR] Failed to load HzAnticheat encryption key:", error);
        ...
        originalOnNet(eventName, (...args) => {
            if (IsDuplicityVersion()) { 
                ...
                if (GetPlayerPing(src) > 0) {
                    ...
                    if (exports["HzAnticheat"] && exports["HzAnticheat"].module_punish) {
...
function handleExplosionEvent(originalFunction) {
    ...
    return function(...args) {
        ...
        if (!IsDuplicityVersion() && isValidResource(resourceName)) {
            emitNet("HzAnticheat:Explosions:Whitelist", {
