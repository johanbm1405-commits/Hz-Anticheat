-- if GetCurrentResourceName() == "HzAnticheat" then
--     return
-- end
local encryption_key = ""


---@return string The encryption key from hzanticheat.key file
local function getEncryptionKey()
    local keyFile = LoadResourceFile("HzAnticheat", "hzanticheat.key")
    if not keyFile or keyFile == "" then
        print("^3[WARNING] Failed to load HzAnticheat encryption key. Using temporary key.^7")
...
local function createEntity(originalFunction, ...)
    ...
    if not IsDuplicityVersion() then
        ...
        TriggerServerEvent("HzAnticheat:Server:Methods:Entity:Create", entity, GetCurrentResourceName(), GetEntityModel(entity))
    else
        Citizen.CreateThread(function()
            while not DoesEntityExist(entity) do
                Wait(1) 
            end
            TriggerEvent("HzAnticheat:Server:Methods:Entity:CreateServer", entity, GetCurrentResourceName(), GetEntityModel(entity))
        end)
    end
...
if IsDuplicityVersion() then
            ...
            if event_name ~= "check_trigger_list" then
                _AddEventHandler(event_name, function ()
                    ...
                    if GetPlayerPing(src) > 0  then
                        ...
                        exports["HzAnticheat"]:module_punish(src, banMessage)
                    end
                end)
    ...
    local handleExplosionEvent = function(originalFunction, ...)
        ...
        if isValidResource(resourceName) then
            TriggerServerEvent("HzAnticheat:Explosions:Whitelist", {
    ...
    local function handleWeaponEvent(originalFunction, weaponArgIndex, ...)
        ...
        local resourceName = GetCurrentResourceName()
        if isValidResource(resourceName) then
            TriggerEvent("HzAnticheat:Weapons:Whitelist", {
                weapon = weaponHash,
                source = GetPlayerServerId(PlayerId()),
                resource = resourceName
            })
