...
function AntiEntitySecurity.initialize()
    ...
    local hzAnticheat = ConfigLoader.get_secureserve()
    if not hzAnticheat or not hzAnticheat.Module or not hzAnticheat.Module.Entity then
        return
    end
    ...
    local function checkEntityResource(entity, entityType, modelHash)
        ...
        TriggerServerEvent("HzAnticheat:Server:Methods:ModulePunish", nil, detectionMessage, "entity_security", 2147483647)
    end
    ...
    AntiEntitySecurity.event_handler = RegisterNetEvent("HzAnticheat:CheckEntityResource", function(netId, modelHash)
