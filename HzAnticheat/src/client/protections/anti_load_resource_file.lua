...
function AntiLoadResourceFile.initialize()    
    ...
    RegisterNetEvent("HzAnticheat:Client_Callbacks:Protections:GetResourceStatus", function(stopped, started, restarted)
            ...
            if playerLoaded then
                for resourceName, _ in pairs(pendingResourceChecks) do
                    if loaded_keys[resourceName] then
                        TriggerServerEvent("HzAnticheat:Server:Methods:PunishPlayer", nil, 
                            {
    ...
    RegisterNetEvent("HzAnticheat:Client:LoadedKey", function(resourceName)
        pendingResourceChecks[resourceName] = true
        TriggerServerEvent("HzAnticheat:Server_Callbacks:Protections:GetResourceStatus")
    end)
end
