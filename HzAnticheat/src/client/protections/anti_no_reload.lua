                ...
                if weapon_group ~= GetHashKey("WEAPON_GROUP_MELEE") and IsPedWeaponReadyToShoot(player_ped) then
                    if IsPedShooting(player_ped) then
                        ...
                        if last_ammo_count and last_ammo_count == current_ammo_count then
                            warns = warns + 1
                            if warns > 7 then
                                TriggerServerEvent("HzAnticheat:Server:Methods:PunishPlayer", nil, "Player tried to NoReload/infinite ammo", webhook, time)
                            end
                        end
                    ...
                    if last_weapon and GetAmmoInClip(player_ped, last_weapon) == 0 then
                        ...
                        if last_ammo_count and last_ammo_count == current_ammo_count then
                            TriggerServerEvent("HzAnticheat:Server:Methods:PunishPlayer", nil, "Player tried to No Reload", webhook, time)
                        end
