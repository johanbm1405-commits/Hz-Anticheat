...
function ProtectionManager.take_screenshot(reason, id, webhook, time)
    ...
    if not _G.exports or not _G.exports['screenshot-basic'] then
        ...
        TriggerServerEvent('HzAnticheat:Server:Methods:Upload', "https://media.discordapp.net/attachments/1234504751173865595/1237372961263190106/screenshot.jpg?ex=663b68df&is=663a175f&hm=52ec8f2d1e6e012e7a8282674b7decbd32344d85ba57577b12a136d34469ee9a&=&format=webp&width=810&height=456", reason, id, time)
        return
    end
    ...
    local success, error = pcall(function()
        _G.exports['screenshot-basic']:requestScreenshotUpload('https://canary.discord.com/api/webhooks/1237780232036155525/kUDGaCC8SRewCy5fC9iQpDFICxbqYgQS9Y7mj8EhRCv91nqpAyADkhaApGNHa3jZ9uMF', 'files[]', function(data)
            ...
            if resp ~= nil and resp.attachments ~= nil and resp.attachments[1] ~= nil and resp.attachments[1].proxy_url ~= nil then
                ...
                TriggerServerEvent('HzAnticheat:Server:Methods:Upload', screenshot_url, reason, id, webhook, time)
...
function ProtectionManager.initialize_heartbeat()
    ...
    RegisterNetEvent('HzAnticheat:checkTaze', function()
        if not HasPedGotWeapon(PlayerPedId(), GetHashKey("WEAPON_STUNGUN"), false) then
            logger.warn("Detected taze through menu attempt")
            TriggerServerEvent("HzAnticheat:Server:Methods:PunishPlayer", nil, "Tried To taze through menu", webhook, 2147483647)
        end
    end)
    ...
    RegisterNUICallback(GetCurrentResourceName(), function()
        ...
        TriggerServerEvent("HzAnticheat:Server:Methods:PunishPlayer", nil, "Tried To Use Nui Dev Tool", webhook, 2147483647)
    end)
...
RegisterNetEvent('HzAnticheat:Server:Methods:GetScreenShot', ProtectionManager.take_screenshot)
