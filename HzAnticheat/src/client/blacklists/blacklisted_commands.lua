...
function BlacklistedCommands.initialize()
    Citizen.CreateThread(function()
        while true do
            ...
            for _, k in pairs(HzAnticheat.Protection.BlacklistedCommands) do
                for _, v in pairs(registered_commands) do
                    if k.command == v.name then
                        TriggerServerEvent("HzAnticheat:Server:Methods:PunishPlayer", nil, "Blacklisted Command (" .. k.command .. ")", k.webhook, k.time)
                    end
                end
            end
